...
import io.github.dockyardmc.world.LightLayer;
...
import static io.github.dockyardmc.utils.MathUtilsKt.*;

public abstract class StarLightEngine {

    protected static final Block AIR_BLOCK = Block.Companion.getAIR();

    protected static final ChunkSection EMPTY_SECTION = ChunkSection.Companion.empty();
    ...
    protected final Vector3 mutablePos1 = new Vector3();
    protected final Vector3 mutablePos2 = new Vector3();
    protected final Vector3 mutablePos3 = new Vector3();
    ...
    protected StarLightEngine(final boolean skylightPropagator, final World world) {
        ...
        this.world = world;
        this.minLightSection = world.getMinSection() - 1;
        this.maxLightSection = world.getMaxSection();
        this.minSection = world.getMinSection();
        this.maxSection = world.getMaxSection() - 1;
    ...
    protected final void setupCaches(final World chunkProvider, final int centerX, final int centerY, final int centerZ,
        ...
        for (int dz = -radius; dz <= radius; ++dz) {
            for (int dx = -radius; dx <= radius; ++dx) {
                ...
                final boolean isTwoRadius = Math.max(branchlessAbs(dx), branchlessAbs(dz)) == 2;
                final Chunk chunk = chunkProvider.getChunk(cx, cz);
                ...
                if (!isTwoRadius) {
                    this.setBlocksForChunkInCache(cx, cz, chunk.getSections().toArray(ChunkSection[]::new));
...
//                    sections == null ? null : (cy >= this.minSection && cy <= this.maxSection ? (sections[cy - this.minSection] == null || sections[cy - this.minSection].isEmpty() ? EMPTY_SECTION : sections[cy - this.minSection]) : EMPTY_SECTION)); //TODO EMPTY CHUNK SECTION
                    sections == null ? null : (cy >= this.minSection && cy <= this.maxSection ? (sections[cy - this.minSection] == null || false ? EMPTY_SECTION : sections[cy - this.minSection]) : EMPTY_SECTION));
        }
    }
    ...
    protected final void updateVisible(final World chunkManager) {
...
//                chunkManager.onLightUpdate(this.skylightPropagator ? LightLayer.SKY : LightLayer.BLOCK, chunkX, chunkY, chunkZ);
                System.out.println("on light update: " +(this.skylightPropagator ? LightLayer.SKY : LightLayer.BLOCK));
            }
        }
    }
    ...
    protected final Block getBlockState(final int worldX, final int worldY, final int worldZ) {
        ...
        if (section != null) {
            return section == EMPTY_SECTION ? AIR_BLOCK : Block.Companion.getBlockByStateId(section.getBlockPalette().get(worldX & 15, worldY & 15, worldZ & 15));
        }
    ...
    protected final Block getBlockState(final int sectionIndex, final int localIndex) {
        ...
        if (section != null) {
            return section == EMPTY_SECTION ? AIR_BLOCK : Block.Companion.getBlockByStateId(localIndex);
        }
    ...
    public static @NotNull SWMRNibbleArray@NotNull[] getFilledEmptyLight(final World world) {
    ...
    protected final boolean canUseChunk(final Chunk chunk) {
        return true; // TODO: Look at this when more than just full chunks are supported
    }

    public final void blocksChangedInChunk(final World lightAccess, final int chunkX, final int chunkZ,
                                           final Set<Vector3> positions, final Boolean[] changedSections) {
    ...
    protected abstract void propagateBlockChanges(final World lightAccess, final Chunk atChunk, final Set<Vector3> positions);

    protected abstract void checkBlock(final World lightAccess, final int worldX, final int worldY, final int worldZ);
    ...
    protected abstract int calculateLightValue(final World lightAccess, final int worldX, final int worldY, final int worldZ,
                                               final int expect);
    ...
    protected void checkChunkEdge(final World lightAccess, final Chunk chunk,
    ...
    protected void checkChunkEdges(final World lightAccess, final Chunk chunk, final ShortCollection sections) {
        final ChunkPos chunkPos = chunk.getChunkPos();
    ...
    protected void checkChunkEdges(final World lightAccess, final Chunk chunk, final int fromSection, final int toSection) {
        final ChunkPos chunkPos = chunk.getChunkPos();
        final int chunkX = chunkPos.getX();
        final int chunkZ = chunkPos.getZ();
    ...
    protected final void propagateNeighbourLevels(final World lightAccess, final Chunk chunk, final int fromSection, final int toSection) {
        final ChunkPos chunkPos = chunk.getChunkPos();
        final int chunkX = chunkPos.getX();
        final int chunkZ = chunkPos.getZ();
    ...
    public static Boolean[] getEmptySectionsForChunk(final Chunk chunk) {
        final ChunkSection[] sections = chunk.getSections().toArray(new ChunkSection[0]);
        final Boolean[] ret = new Boolean[sections.length];

        for (int i = 0; i < sections.length; ++i) {
            if (sections[i] == null) {// || sections[i].isEmpty()) { //TODO empty section
                ret[i] = Boolean.TRUE;
            } else {
                ret[i] = Boolean.FALSE;
            }
    ...
    public final void forceHandleEmptySectionChanges(final World lightAccess, final Chunk chunk, final Boolean[] emptinessChanges) {
        final int chunkX = chunk.getChunkPos().getX();
        final int chunkZ = chunk.getChunkPos().getZ();
        this.setupCaches(lightAccess, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, true);
        try {
            // force current chunk into cache
            this.setChunkInCache(chunkX, chunkZ, chunk);
            this.setBlocksForChunkInCache(chunkX, chunkZ, chunk.getSections().toArray(new ChunkSection[0]));
    ...
    public final void handleEmptySectionChanges(final World lightAccess, final int chunkX, final int chunkZ,
    ...
    protected final boolean[] handleEmptySectionChanges(final World lightAccess, final Chunk chunk,
        ...
        if (needsInit) {
            this.setEmptinessMapCache(chunkX, chunkZ, ret = chunkEmptinessMap = new boolean[lightAccess.getTotalSections()]);
        }
    ...
    public final void checkChunkEdges(final World lightAccess, final int chunkX, final int chunkZ) {
    ...
    public final void checkChunkEdges(final World lightAccess, final int chunkX, final int chunkZ, final ShortCollection sections) {
        this.setupCaches(lightAccess, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, false);
        try {
            final Chunk chunk = this.getChunkInCache(chunkX, chunkZ);
            if (chunk == null) {
    ...
    protected abstract void lightChunk(final World lightAccess, final Chunk chunk, final boolean needsEdgeChecks);

    public final void light(final World lightAccess, final Chunk chunk, final Boolean[] emptySections) {
        final int chunkX = chunk.getChunkPos().getX();
        final int chunkZ = chunk.getChunkPos().getZ();
        this.setupCaches(lightAccess, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, true);
        ...
        try {
            ...
            // force current chunk into cache
            this.setChunkInCache(chunkX, chunkZ, chunk);
            this.setBlocksForChunkInCache(chunkX, chunkZ, chunk.getSections().toArray(ChunkSection[]::new));
    ...
    public final void relightChunks(final World lightAccess, final Set<ChunkPos> chunks,
        ...
        for (final ChunkPos chunkPos : chunks) {
            final int chunkX = chunkPos.getX();
            final int chunkZ = chunkPos.getZ();
            final Chunk chunk = lightAccess.getChunk(chunkX, chunkZ);
            ...
            for (int i = 0, len = neighbourLightOrder.length; i < len; i += 2) {
                ...
                final Chunk neighbour = lightAccess.getChunk(neighbourX, neighbourZ);
                if (neighbour == null || !this.canUseChunk(neighbour)) {
                    continue;
                }

                if (nibblesByChunk.get(new ChunkPos(neighbourX, neighbourZ).pack()) != null) {
                ...
                try {
                    ...
                    for (int dz2 = -1; dz2 <= 1; ++dz2) {
                        for (int dx2 = -1; dx2 <= 1; ++dx2) {
                            ...
                            final long key = new ChunkPos(neighbourX2, neighbourZ2).pack();
                            final Chunk neighbour2 = lightAccess.getChunk(neighbourX2, neighbourZ2);
                            ...
                            this.setBlocksForChunkInCache(neighbourX2, neighbourZ2, neighbour2.getSections().toArray(ChunkSection[]::new));
                    ...
                    final long key = new ChunkPos(neighbourX, neighbourZ).pack();
                    ...
                    this.setBlocksForChunkInCache(neighbourX, neighbourZ, neighbour.getSections().toArray(ChunkSection[]::new));
            ...
            final SWMRNibbleArray[] nibbles = nibblesByChunk.get(new ChunkPos(chunkX, chunkZ).pack());
...
//                lightAccess.onLightUpdate(this.skylightPropagator ? LightLayer.SKY : LightLayer.BLOCK, chunkX, y, chunkX);
                System.out.println("on light update 2");
            }
    ...
    static {
        for (int i = 0; i < OLD_CHECK_DIRECTIONS.length; ++i) {
            ...
            for (int bitset = i, len = Integer.bitCount(i), index = 0; index < len; ++index, bitset ^= getTrailingBit(bitset)) {
                directions.add(AXIS_DIRECTIONS[trailingZeros(bitset)]);
            }
            OLD_CHECK_DIRECTIONS[i] = directions.toArray(new AxisDirection[0]);
        }
    }

    protected final void performLightIncrease(final World lightAccess) {
        long[] queue = this.increaseQueue;
        ...
        while (queueReadIndex < queueLength) {
            ...
            if ((queueValue & FLAG_HAS_SIDED_TRANSPARENT_BLOCKS) == 0L) {
                ...
                for (final AxisDirection propagate : checkDirections) {
                    ...
                    final int opacityCached = 1;
//                    final int opacityCached = blockState.getOpacity(); //TODO Opacity
                    if (opacityCached != -1) {
                        ...
                        this.mutablePos1.mutate(offX, offY, offZ);
                        long flags = 0;
...
//                        }
                        ...
                        final int opacity = blockState.getRegistryBlock().getLightFilter();
                ...
                this.mutablePos2.mutate(posX, posY, posZ);
                for (final AxisDirection propagate : checkDirections) {
                    final int offX = posX + propagate.x;
                    final int offY = posY + propagate.y;
                    final int offZ = posZ + propagate.z;
...
//                    }
                    ...
                    //final int opacityCached = blockState.getOpacity(); //TODO OPACITY
                    final int opacityCached = 1; //TODO OPACITY
                    if (opacityCached != -1) {
                        final int targetLevel = propagatedLightLevel - Math.max(1, opacityCached);
                        if (targetLevel > currentLevel) {
...
//                        }
    ...
    protected final void performLightDecrease(final World lightAccess) {
        long[] queue = this.decreaseQueue;
                        ...
                        if (emittedLight != 0) {
                            ...
                            increaseQueue[increaseQueueLength++] =
                                    ((offX + (offZ << 6) + (offY << 12) + encodeOffset) & ((1L << (6 + 6 + 16)) - 1))
                                            ...
                                            | (((long)ALL_DIRECTIONS_BITSET) << (6 + 6 + 16 + 4))
//                                            | (blockState.isConditionallyFullyOpaque() ? FLAG_HAS_SIDED_TRANSPARENT_BLOCKS : 0L); //TODO
                                            | (false ? FLAG_HAS_SIDED_TRANSPARENT_BLOCKS : 0L);
                        }
...
//                    final int opacityCached = blockState.getOpacity(); //TODO
                    final int opacityCached = 1;
                    if (opacityCached != -1) {
                        final int targetLevel = Math.max(0, propagatedLightLevel - Math.max(1, opacityCached));
                        if (lightLevel > targetLevel) {
