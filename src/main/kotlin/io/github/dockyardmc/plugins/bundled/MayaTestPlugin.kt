...
class MayaTestPlugin: DockyardPlugin {
    ...
    override fun load(server: DockyardServer) {
        ...
        Commands.add("/cube") {
            it.execute { exec ->
                ...
                showCube = !showCube
                player.sendMessage("<yellow>Toggled cube visibility")
            }
        }
    }
    ...
    // ik inefficient but is just demo
    fun drawLine(start: Location, end: Location, world: World) {
        ...
        for (i in 0..(distance / step).toInt()) {
            if(showCube) world.spawnParticle(currentLocation, Particles.ELECTRIC_SPARK, count = 1, speed = 0f)
