...
object FrogVariantRegistry : DataDrivenRegistry, DynamicRegistry {

    override val identifier: String = "minecraft:frog_variant"

    private lateinit var cachedPacket: ClientboundRegistryDataPacket
    val pigVariants: MutableMap<String, FrogVariant> = mutableMapOf()
    ...
    override fun initialize(inputStream: InputStream) {
        ...
        val list = Json.decodeFromStream<List<FrogVariant>>(stream)
        list.forEach { entry -> addEntry(entry, false) }
        updateCache()
    }

    fun addEntry(entry: FrogVariant, updateCache: Boolean = true) {
...
data class FrogVariant(
    ...
    override fun getProtocolId(): Int {
        return FrogVariantRegistry.protocolIds.getOrThrow(identifier)
    }
