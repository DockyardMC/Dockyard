...
import io.github.dockyardmc.world.light.LightEngine.CONTENT_FULLY_LIT
import io.github.dockyardmc.world.light.LightEngine.SECTION_SIZE
...
class SkyLight : Light {
    ...
    companion object {
        ...
        fun buildInternalQueue(heightmap: IntArray, maxY: Int, sectionY: Int): ShortArrayFIFOQueue {
            val lightSources = ShortArrayFIFOQueue()
            val sectionMaxY = (sectionY + 1) * 16 - 1
            val sectionMinY = sectionY * 16

            for (x in 0 until 16) {
                for (z in 0 until 16) {
                    val height: Int = heightmap[z shl 4 or x]
                    for (y in min(sectionMaxY, maxY) downTo max(height, sectionMinY)) {
                        val index = x or (z shl 4) or ((y % 16) shl 8)
                        lightSources.enqueue((index or (15 shl 12)).toShort())
                    }
                }
            }
            return lightSources
    ...
    override fun calculateInternal(
        ...
        this.isValidBorders = true
        var queueSize: Int = SECTION_SIZE * SECTION_SIZE * SECTION_SIZE
        var queue = ShortArrayFIFOQueue(0)
        ...
        for (i in -1..1) {
            for (j in -1..1) {
                for (k in -1..1) {
                    ...
                    val neighborY = chunkY + j
                    val neighborZ: Int = chunkPos.z + k

                    val light = lookup.light(neighborX, neighborY, neighborZ)
                    if (light !is SkyLight) continue

                    light.contentPropagation = null
                    toUpdate.add(Vector3(neighborX, neighborY, neighborZ).toVector3d())
                }
            }
        }
    ...
    override fun calculateExternal(
        ...
        var contentPropagationTemp = CONTENT_FULLY_LIT
        if (!fullyLit) {
            val queue = buildExternalQueue(blockPalette, neighbours, content, lightLookup, paletteLookup)
            contentPropagationTemp = LightEngine.compute(blockPalette, queue)
