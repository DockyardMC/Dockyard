...
object BannerPatternRegistry: DynamicRegistry {
    ...
    override fun getCachedPacket(): ClientboundRegistryDataPacket {
        if(!BannerPatternRegistry::cachedPacket.isInitialized) updateCache()
        return cachedPacket
    }
    ...
    override fun get(identifier: String): BannerPattern {
        return bannerPatterns[identifier] ?: throw throw RegistryException(identifier, getMap().size)
    }
    ...
    override fun getByProtocolId(id: Int): BannerPattern {
        return bannerPatterns.values.toList().getOrNull(id) ?: throw throw RegistryException(identifier, getMap().size)
    }
