...
object EntityTypeRegistry: DataDrivenRegistry {
    ...
    override fun get(identifier: String): EntityType {
        return entityTypes[identifier] ?: throw RegistryException(identifier, getMap().size)
    }
    ...
    override fun getByProtocolId(id: Int): EntityType {
        val identifier = protocolIds.reversed()[id] ?: throw RegistryException(id, getMap().size)
...
data class EntityDimensions(
    ...
    val height: Float,
    val width: Float,
    val nameTagLocation: NetworkVector3d?,
    val passengerLocations: List<NetworkVector3d>?,
    val vehicleLocation: NetworkVector3d?,
    val wardenChestLocation: NetworkVector3d?
)

@Serializable
data class NetworkVector3d(val x: Double, val y: Double, val z: Double)