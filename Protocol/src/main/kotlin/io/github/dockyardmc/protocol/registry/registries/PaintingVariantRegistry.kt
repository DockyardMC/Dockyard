...
object PaintingVariantRegistry: DynamicRegistry {
    ...
    override fun getCachedPacket(): ClientboundRegistryDataPacket {
        if(!PaintingVariantRegistry::cachedPacket.isInitialized) updateCache()
        return cachedPacket
    }
    ...
    override fun get(identifier: String): PaintingVariant {
        return paintingVariants[identifier] ?: throw RegistryException(identifier, getMap().size)
    }
    ...
    override fun getByProtocolId(id: Int): PaintingVariant {
        return paintingVariants.values.toList().getOrNull(id) ?: throw RegistryException(id, getMap().size)
    }
