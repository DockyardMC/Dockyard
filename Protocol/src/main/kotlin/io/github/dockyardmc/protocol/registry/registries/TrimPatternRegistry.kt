...
object TrimPatternRegistry: DynamicRegistry {
    ...
    override fun getCachedPacket(): ClientboundRegistryDataPacket {
        if(!TrimPatternRegistry::cachedPacket.isInitialized) updateCache()
        return cachedPacket
    }
    ...
    override fun get(identifier: String): TrimPattern {
        return trimPatterns[identifier] ?: throw RegistryException(identifier, getMap().size)
    }
    ...
    override fun getByProtocolId(id: Int): TrimPattern {
        return trimPatterns.values.toList().getOrNull(id) ?: throw RegistryException(id, getMap().size)
    }
