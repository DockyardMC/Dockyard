...
object DamageTypeRegistry: DynamicRegistry {
    ...
    override fun register() {
        ...
        addEntry(DamageType("minecraft:is_fire", effects = "burning", exhaustion = 0.0f, scaling = "when_caused_by_living_non_player", messageId = "onFire"), false)
    ...
    override fun getCachedPacket(): ClientboundRegistryDataPacket {
        if(!DamageTypeRegistry::cachedPacket.isInitialized) updateCache()
        return cachedPacket
    }
    ...
    override fun get(identifier: String): DamageType {
        return damageTypes[identifier] ?: throw RegistryException(identifier, getMap().size)
    }
    ...
    override fun getByProtocolId(id: Int): DamageType {
        return damageTypes.values.toList().getOrNull(id) ?: throw RegistryException(id, getMap().size)
    }
